...
/* Adjust color mix functions rounding. GPUs might calculate (x >> 1) + ((x >>
 * 1) + 1) differently. 0: round down, 64: round up from x.75, 128: round up
 * from half, 192: round up from x.25, 255: round up */
...
#if LV_MEM_ADR == 0
// #define LV_MEM_POOL_INCLUDE your_alloc_library  /* Uncomment if using an
// external allocator*/ #define LV_MEM_POOL_ALLOC   your_alloc          /*
// Uncomment if using an external allocator*/
#endif
 ...
 * internal calculation. (0: use malloc, 1: use a fixed static memory buffer)*/
...
/*Default display refresh period. `LV_DISP_DEF_REFR_PERIOD` is the default time
 * (in milliseconds) between screen refreshes*/
 ...
 *Required to draw shadow, gradient, rounded corners, circles, arc, skew, image
 ...
 *When LVGL calculates the gradient "maps" it can save them into a cache to
 * avoid calculating them again. LV_GRAD_CACHE_DEF_SIZE sets the size of this
 * cache in bytes. If the cache is too small the map will be allocated only
 * while it's required for the drawing. 0 mean no caching.*/
#define LV_GRAD_CACHE_DEF_SIZE 0

/*Allow dithering the gradients (to achieve visual smooth color gradients on
 * limited color depth display) LV_DITHER_GRADIENT implies LV_GRADIENT_MAX_STOPS
 * = 256 is defined. 0: Disable dithering 1: Enable dithering*/
 ...
 * lv_init(). Note that symbol SDK_OS_FREE_RTOS has to be defined in order to
 * use FreeRTOS OSA, otherwise bare-metal implementation is selected. 0:
...
/*Custom blend mode for mask drawing, disable if you need to link with older
 * SDL2 lib*/
 ...
 * problem LV_LOG_LEVEL_ERROR       Only critical issue, when the system may
 * fail LV_LOG_LEVEL_USER        Only logs added by the user LV_LOG_LEVEL_NONE
 *      Do not log anything*/
...
#define LV_USE_ASSERT_STYLE                                                    \
  0 /*Check if the used styles are properly initialized. (Very fast,           \
       recommended)*/
 ...
 *0: Disable custom handler (default)
 *1: Enable custom handler*/
 ...
 *Used if lvgl is bound to higher level language and the memory is managed by
 * that language*/
 ...
 * texts. The direction will be processed according to the Unicode Bidirectional
...
#define LV_FS_STDIO_LETTER                                                     \
  '\0' /*Set an upper cased letter on which the drive will accessible (e.g.    \
          'A')*/
...
/* sbit cache:it is much more memory efficient for small bitmaps(font size <
 * 256) */
/* if font size >= 256, must use image cache */
#define LV_FREETYPE_SBIT_CACHE 0
/* Maximum number of opened FT_Face/FT_Size objects managed by this cache
 * instance. */
...
/*1: Enable API to take snapshots for object*/
...
/*If you do not use the default thesaurus, please use `lv_ime_pinyin` after
 * setting the thesauruss*/
#define LV_IME_PINYIN_USE_DEFAULT_DICT 1
/*If you use the default thesaurus, you can use the following macro to change
 * the size of the thesaurus*/
/*If you do not use the default thesaurus, you can use the following macro to
 * change the size of the thesaurus*/
#define LV_IME_PINYIN_CAND_TEXT_NUM 6

/*The maximum number of Chinese characters that can be displayed*/
/*This needs to be adjusted according to the size of the thesaurus*/
#define LV_IME_PINYIN_CAND_TEXT_MAX_LEN 4
#endif

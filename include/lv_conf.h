 ...
 * tasks. You will see an error log message if there wasn't enough buffers. */
 ...
 *If only the image size is modified and the datas stored are not changed, the
 * cached image will be used instead of opening it again. */
 ...
 *Error diffusion dithering gets a much better visual result, but implies more
 * CPU consumption and memory when drawing. The increase in memory consumption
 * is (32 bits * object width size)*/
 ...
 * (lv_gpu_nxp_pxp_irq.c) and call lv_gpu_nxp_pxp_init() automatically during
 * lv_init(). Note that symbol SDK_OS_FREE_RTOS has to be defined in order to
 * use FreeRTOS OS, otherwise bare-metal implementation is selected. 0:
 ...
 * 256) */
/* image cache:it is much more memory efficient for large bitmaps(font size >=
 * 256) */
